...
export function mountApp(container, options) {
    ...
    let dificultadSeleccionada = options.dificultad ?? "media";
    let guessInputValue = "";
    ...
    `;
        container.querySelectorAll(".difficulty-button").forEach((button) => {
            button.addEventListener("click", () => {
                const modo = button.dataset.dificultad;
                dificultadSeleccionada = modo ?? "media";
                window.localStorage.setItem("ahorcado-dificultad", dificultadSeleccionada);
                render();
            });
        });
        const inicio = container.querySelector("[data-testid='start-button']");
        if (inicio) {
    ...
    `;
        const input = container.querySelector('[data-testid="guess-input"]');
        const button = container.querySelector('[data-testid="guess-button"]');
        function procesarAdivinanza() {
            if (!input || !juego)
                return;
